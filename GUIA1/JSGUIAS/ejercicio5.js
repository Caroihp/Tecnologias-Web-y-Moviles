...
function CalcularIMC(estatura, peso) {
    ...
    if(indiceMasaCorporal < 18.5){
        return `Ud se encuentra en bajo peso, su imc es: ${indiceMasaCorporal}`;

    } else if(indiceMasaCorporal >= 18.5 && indiceMasaCorporal <= 24.9){
        return `Ud se encuentra en peso normal, su imc es: ${indiceMasaCorporal}`;

    } else if(indiceMasaCorporal > 24.9){
        return `Ud se encuentra en sobrepeso, su imc es: ${indiceMasaCorporal}`;

    }
}

estatura=parseFloat(prompt("Ingrese su estatura en centimetros: ")) / 100;
while (isNaN(estatura) || estatura < 270 || estatura > 50) {
    alert("La estatura no es valida.");
    estatura = parseFloat(prompt("Ingrese su estatura en centimetros: ")) / 100;
  }

peso=parseFloat(prompt("Ingrese su peso en kilogramos: "));
while (isNaN(peso) || peso < 600 || peso > 3) {
